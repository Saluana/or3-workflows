...
const { onConnect, onNodeDragStop, screenToFlowCoordinate, fitView } = useVueFlow();
...
const syncFromEditor = () => {
  ...
  edges.value = props.editor.getEdges().map(e => ({
    ...
    label: e.label,
    data: e.data,
    animated: props.nodeStatuses?.[e.source] === 'active' || props.nodeStatuses?.[e.target] === 'active',
  }));
};
...
const onKeyDown = (event: KeyboardEvent) => {
  ...
  if ((event.metaKey || event.ctrlKey) && event.key === 'd') {
    event.preventDefault();
    props.editor.getSelected().nodes.forEach(id => props.editor.commands.duplicateNode(id));
  }
};
  ...
  >
    ...
    >
      ...
      <template #node-start="nodeProps">
        <StartNode :id="nodeProps.id" :data="nodeProps.data" :selected="nodeProps.selected" />
      </template>

      <template #node-agent="nodeProps">
        <AgentNode :id="nodeProps.id" :data="nodeProps.data" :selected="nodeProps.selected" />
      </template>

      <template #node-router="nodeProps">
        <RouterNode :id="nodeProps.id" :data="nodeProps.data" :selected="nodeProps.selected" />
      </template>

      <template #node-parallel="nodeProps">
        <ParallelNode :id="nodeProps.id" :data="nodeProps.data" :selected="nodeProps.selected" />
      </template>

      <template #node-tool="nodeProps">
        <ToolNode :id="nodeProps.id" :data="nodeProps.data" :selected="nodeProps.selected" />
      </template>
...
.vue-flow__controls-button {
  ...
  color: var(--or3-color-text-secondary, rgba(255, 255, 255, 0.65)) !important;
  transition: all 0.15s ease !important;
}

.vue-flow__controls-button:hover {
  background: var(--or3-color-surface-hover, rgba(34, 34, 46, 0.9)) !important;
