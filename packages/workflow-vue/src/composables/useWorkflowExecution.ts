  ...
  const execute = async (
    ...
    const callbacks: ExecutionCallbacks = {
      onNodeStart: (nodeId) => {
        ...
        nodeStatuses.value = { ...nodeStatuses.value, [nodeId]: 'active' };
        externalCallbacks?.onNodeStart?.(nodeId);
      },
      onNodeFinish: (nodeId, output) => {
        nodeStatuses.value = { ...nodeStatuses.value, [nodeId]: 'completed' };
      ...
      onNodeError: (nodeId, err) => {
        nodeStatuses.value = { ...nodeStatuses.value, [nodeId]: 'error' };
        error.value = err;
    ...
    try {
      const executionResult = await adapter.execute(workflow, input, callbacks);
