...
describe('validateWorkflow', () => {
  describe('start node validation', () => {
    ...
    it('should pass with exactly one start node', () => {
      ...
      expect(result.errors.filter(e => e.message.includes('Start'))).toHaveLength(0);
    });
  });
  ...
  describe('connectivity validation', () => {
    it('should error when node is completely disconnected', () => {
      const nodes = [
        { id: 'start-1', type: 'start', data: { label: 'Start' } },
        { id: 'agent-1', type: 'agent', data: { label: 'Lonely Agent' } },
        { id: 'agent-2', type: 'agent', data: { label: 'Connected Agent' } },
      ];
    ...
    it('should pass when all nodes are connected', () => {
      ...
      expect(result.errors.filter(e => e.message.includes('connected'))).toHaveLength(0);
    });
  });
  ...
  describe('agent node validation', () => {
    it('should warn when agent has no prompt', () => {
      const nodes = [
        { id: 'start-1', type: 'start', data: { label: 'Start' } },
        { id: 'agent-1', type: 'agent', data: { label: 'No Prompt Agent' } },
      ];
      const edges = [{ id: 'e1', source: 'start-1', target: 'agent-1' }];
    ...
    it('should warn when agent has empty prompt', () => {
      const nodes = [
        { id: 'start-1', type: 'start', data: { label: 'Start' } },
        { id: 'agent-1', type: 'agent', data: { label: 'Empty Prompt', prompt: '   ' } },
      ];
      const edges = [{ id: 'e1', source: 'start-1', target: 'agent-1' }];
    ...
    it('should pass when agent has prompt', () => {
      const nodes = [
        { id: 'start-1', type: 'start', data: { label: 'Start' } },
        { id: 'agent-1', type: 'agent', data: { label: 'Agent', prompt: 'You are helpful' } },
      ];
      const edges = [{ id: 'e1', source: 'start-1', target: 'agent-1' }];

      const result = validateWorkflow(nodes, edges);

      expect(result.warnings.filter(w => w.nodeId === 'agent-1')).toHaveLength(0);
    });
  });
  ...
  describe('cycle detection', () => {
    ...
    it('should not warn for acyclic workflow', () => {
      const nodes = [
        { id: 'start-1', type: 'start', data: { label: 'Start' } },
        { id: 'agent-1', type: 'agent', data: { label: 'Agent 1', prompt: 'test' } },
        { id: 'agent-2', type: 'agent', data: { label: 'Agent 2', prompt: 'test' } },
      ];
      const edges = [
        { id: 'e1', source: 'start-1', target: 'agent-1' },
        { id: 'e2', source: 'agent-1', target: 'agent-2' },
  ...
  describe('overall validation', () => {
    it('should return isValid=true for valid workflow', () => {
      const nodes = [
        { id: 'start-1', type: 'start', data: { label: 'Start' } },
        { id: 'agent-1', type: 'agent', data: { label: 'Agent', prompt: 'test' } },
      ];
      const edges = [{ id: 'e1', source: 'start-1', target: 'agent-1' }];
    ...
    it('should collect multiple errors and warnings', () => {
      const nodes = [
        { id: 'agent-1', type: 'agent', data: { label: 'Disconnected' } },
        { id: 'agent-2', type: 'agent', data: { label: 'Also Disconnected' } },
      ];
      const edges: { id: string; source: string; target: string }[] = [];
